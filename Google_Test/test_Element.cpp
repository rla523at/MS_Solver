...
TEST(ReferenceGeometry, mapping_nodes1) {
	const auto fig = Figure::line;
	const auto fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
	const auto result = ref_geo->mapping_nodes();

	const std::vector<Euclidean_Vector> ref = { { -1,0 }, { 1,0 } };
	EXPECT_EQ(ref, result);
}
TEST(ReferenceGeometry, mapping_nodes2) {
	const Figure fig = Figure::triangle;
	const ushort fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
	const auto result = ref_geo->mapping_nodes();
...
TEST(ReferenceGeometry, mapping_nodes3) {
	const Figure fig = Figure::quadrilateral;
	const ushort fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
	const auto result = ref_geo->mapping_nodes();
...
TEST(ReferenceGeometry, connectivities1) {
	const auto fig = Figure::triangle;
	const auto fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);

	constexpr ushort post_order = 0;
	const auto result = ref_geo->connectivities(post_order);

	const std::vector<std::vector<uint>> ref = { {0,1,2} };
	EXPECT_EQ(result, ref);
}
TEST(ReferenceGeometry, connectivities2) {
	const Figure fig = Figure::triangle;
	const ushort fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
...
TEST(ReferenceGeometry, connectivities3) {
	const Figure fig = Figure::quadrilateral;
	const ushort fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
...
TEST(ReferenceGeometry, connectivities4) {
	const Figure fig = Figure::quadrilateral;
	const ushort fig_order = 1;
	const auto ref_geo = Reference_Geometry_Factory::make(fig, fig_order);
...
//	const ReferenceGeometry<space_dimension> simplexref_geometry(ref_fig, fig_order);
//
