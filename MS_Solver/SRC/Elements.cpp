...
{
	auto points = this->points_at_indexes(rearranged_node_indexes);
	this->point_indexes_ = std::move(rearranged_node_indexes);
	this->change_points(std::move(points));
}
...
std::vector<uint> Element::find_periodic_matched_node_indexes(const Element& other) const 
{
	...
	const auto this_num_node = this->point_indexes_.size();
	const auto other_num_node = other.point_indexes_.size();
...
std::vector<Euclidean_Vector> Element::points_at_indexes(const std::vector<uint>& indexes) const 
{
	const auto num_index = indexes.size();
	std::vector<Euclidean_Vector> points(num_index);

	for (ushort i = 0; i < num_index; ++i)
	{
		const auto index_iter = std::find(this->point_indexes_.begin(), this->point_indexes_.end(), indexes[i]);
		REQUIRE(index_iter != this->point_indexes_.end(), "index should be included");

		const auto pos = index_iter - this->point_indexes_.begin();
		points[i] = this->points_[pos];
	}
...
std::vector<std::vector<uint>> Element::set_of_face_node_indexes(void) const 
{
	...
	{
		set_of_face_node_indexes[i] = ms::extract_by_index(this->point_indexes_, set_of_face_node_index_sequences[i]);
	}
