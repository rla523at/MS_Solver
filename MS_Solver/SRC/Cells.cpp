    ...
    : Cells(configuration, grid)
{
    ...
    for (int cell_index = 0; cell_index < this->num_cells_; ++cell_index)
    {
        ...
        this->P0_basis_values_[cell_index] = discrete_solution.calculate_P0_basis_value(cell_index);
        this->set_of_basis_QPs_m_[cell_index] = discrete_solution.calculate_basis_points_m(cell_index, QPs);
...
{
    const auto P0_solutions = discrete_solution.calculate_P0_solutions(this->P0_basis_values_);
...
void Cells_DG::calculate_RHS(double* rhs, const Discrete_Solution_DG& discrete_solution) const
{
    ...
    {
        const auto solution_at_QPs = discrete_solution.calculate_solution_at_points(cell_index, this->set_of_basis_QPs_m_[cell_index]);
