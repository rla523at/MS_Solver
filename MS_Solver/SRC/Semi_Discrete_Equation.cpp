...
std::vector<double> Semi_Discrete_Equation_DG::calculate_global_error_values(const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto L1_norm = this->calculate_global_error_L1_norm(exact_solution, grid, end_time);
	const auto L2_norm = this->calculate_global_error_L2_norm(exact_solution, grid, end_time);
	const auto Linf_norm = this->calculate_global_error_Linf_norm(exact_solution, grid, end_time);
		
	return { L1_norm, L2_norm, Linf_norm };
}

std::vector<double> Semi_Discrete_Equation_DG::calculate_specific_error_values(const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	...
	{
		arithmetic_mean_L1_error += this->calculate_local_error_L1_norm(cell_index, exact_solution, grid, end_time);
		arithmetic_mean_L2_error += this->calculate_local_error_L2_norm(cell_index, exact_solution, grid, end_time);
		arithmetic_mean_Linf_error += this->calculate_local_error_Linf_norm(cell_index, exact_solution, grid, end_time);
	}
...
double Semi_Discrete_Equation_DG::calculate_local_error_L1_norm(const uint cell_index, const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
...
double Semi_Discrete_Equation_DG::calculate_local_error_L2_norm(const uint cell_index, const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto solution_degree = this->discrete_solution_->solution_degree(cell_index);
	const auto integrand_degree = 2 * solution_degree;
	const auto& quadrature_rule = grid.get_cell_quadrature_rule(cell_index, integrand_degree);
...
double Semi_Discrete_Equation_DG::calculate_local_error_Linf_norm(const uint cell_index, const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto solution_degree = this->discrete_solution_->solution_degree(cell_index);
	const auto integrand_degree = 2 * solution_degree;
	const auto& quadrature_rule = grid.get_cell_quadrature_rule(cell_index, integrand_degree);
...
double Semi_Discrete_Equation_DG::calculate_global_error_L1_norm(const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto num_cells = grid.num_cells();
...
double Semi_Discrete_Equation_DG::calculate_global_error_L2_norm(const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto num_cells = grid.num_cells();
...
double Semi_Discrete_Equation_DG::calculate_global_error_Linf_norm(const Exact_Solution& exact_solution, const Grid& grid, const double end_time) const
{
	const auto num_cells = grid.num_cells();
