...
public:
	static void set_path(const std::string& path) { Post_Solution_Data::path_ = path; };	
	static void post_grid(const std::vector<Element<space_dimension_>>& cell_elements);	
	static void syncronize_time(const double& current_time) { Post_Solution_Data::time_ptr_ = &current_time; };
...
template <typename Governing_Equation>
void Post_Solution_Data<Governing_Equation>::post_grid(const std::vector<Element<space_dimension_>>& cell_elements) {
	const size_t num_cell = cell_elements.size();
	Post_Solution_Data::num_post_points_.resize(num_cell);
	...
	for (size_t i = 0; i < num_cell; ++i) {
		...
		Post_Solution_Data::num_post_points_[i] = post_nodes.size();
		...
		connectivity_start_index += Post_Solution_Data::num_post_points_[i];
		Post_Solution_Data::num_node_ += Post_Solution_Data::num_post_points_[i];
		Post_Solution_Data::num_element_ += local_connectivities.size();
	}

	auto grid_post_header_text = Post_Solution_Data::header_text(Post_File_Type::Grid);
...
template <typename Governing_Equation>
void Post_Solution_Data<Governing_Equation>::post_solution(const std::vector<Euclidean_Vector<num_equation_>>& solutions, const std::string& comment) {
	...
	auto solution_post_header_text = Post_Solution_Data::header_text(Post_File_Type::Solution);
	...
	if constexpr (ms::is_SCL_2D<Governing_Equation>) {
		...
		for (size_t i = 0; i < num_solution; ++i) {
			...
			for (size_t j = 0; j < Post_Solution_Data::num_post_points_[i]; ++j, ++str_per_line) {
		...
		for (size_t i = 0; i < num_solution; ++i) {
			...
			for (size_t j = 0; j < Post_Solution_Data::num_post_points_[i]; ++j) {
...
template <typename Governing_Equation>
Text Post_Solution_Data<Governing_Equation>::header_text(const Post_File_Type file_type) {
